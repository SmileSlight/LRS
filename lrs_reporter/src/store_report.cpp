...
StoreReport::StoreReport()
{
    ...
    std::string db_name = config_file::instance()->GetString("mysql", "db_name", "lrs_dns");
...
void StoreReport::store(lrs::ReportStatusRequest req)
{
    for (int i = 0; i < req.results_size(); i++) {
        //一条report 调用记录
        const lrs::HostCallResult &result = req.results(i);
