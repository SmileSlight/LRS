...
void Usage() {
    printf("Usage: ./lrs_dns_test -h ip -p port\n");
}
...
void on_connection(net_connection *conn, void *args) 
{
    ...
    conn->send_message(requestString.c_str(), requestString.size(), lrs::ID_GetRouteRequest);
}
...
int main(int argc, char **argv)
{
    ...
    //创建客户端
    client = new tcp_client(&loop, option.ip, option.port, "lrs_dns_test");
    if (client == NULL) {
    ...
    //设置服务端回应包处理业务
    client->add_msg_router(lrs::ID_GetRouteResponse, deal_get_route);
